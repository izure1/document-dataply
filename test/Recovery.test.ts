...
describe('DocumentDataply Recovery and Backfill Test', () => {
  ...
  test('should rollback index backfill when error occurs during initialization', async () => {
    ...
    let db = DocumentDataply.Define<RecoveryDoc>().Options({ logLevel: 3 }).Open(currentDbPath)
    await db.init()
    ...
    db = DocumentDataply.Define<RecoveryDoc>().Options({ logLevel: 3 }).Open(currentDbPath)
